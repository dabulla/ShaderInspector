...
void Camera::setPosition( const QVector3D& position )
{
    ...
    //d->m_viewCenter = d->m_cameraToCenter + position;
    d->m_cameraToCenter = d->m_viewCenter - position;
    d->m_upVector = QVector3D(0.0,1.0,0.0);
...
{
    Q_D( const Camera );
    return d->m_cameraToCenter.normalized();
}
...
void Camera::translate( const QVector3D& vLocal, CameraTranslationOption option )
{
    ...
    QVector3D oldcameraToCenter = d->m_viewCenter - d->m_position;
    ...
    {
        float oldLen = oldcameraToCenter.length();
        QVector3D newCameraToCenter(d->m_viewCenter - d->m_position);
        if(oldLen < 0.01 && newCameraToCenter.length() < oldLen || QVector3D::dotProduct(oldcameraToCenter, newCameraToCenter) < 0.0)
        {
            d->m_position -= vWorld;
        }
    }
